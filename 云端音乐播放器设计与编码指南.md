...
| 访问权限 | 无登录，接口对访客开放 | 通过曲库清单和短期 B2 签名保护私有对象 |
...
播放器默认无登录；B2 桶仍保持私有，公开接口只为曲库清单中的对象发放短期签名。签名接口公开后要关注滥用、TTL 和费用，不要把“桶是私有的”误认为“签名地址可以永久公开”。
...
4. Function 查清单并校验曲目 id，把对应 B2 对象签为短期 GET URL。
...
| 接口 | 输入 | 成功响应 | 缓存 |
| --- | --- | --- | --- |
| `GET /api/library` | 无 | `{ schemaVersion, updatedAt, tracks: Track[] }` | `no-store` |
| `GET /api/play-url?id=...` | 一个合法曲目 id | `PlayTicket` | `no-store` |

错误统一为 `{ "error": { "code": "TRACK_NOT_FOUND", "message": "歌曲不存在或已移除" } }`。

状态约定：400 为缺失/非法 id，404 为清单没有此歌曲，405 为不支持的方法，500 为配置或签名内部错误。签名接口不访问 B2，因此不把“签名成功”包装成“B2 文件存在”；B2 的 403/404 出现在媒体请求阶段。

前端 API helper 检查 `response.ok` 与 JSON Content-Type。平台错误页或其他非 JSON 响应不能直接当 JSON 解析；出现这种情况保留播放位置并显示可重试的错误。

### 6.2 公开入口

页面和两个 API 对任何访客开放，不开发注册、密码或登录界面。`/api/library` 只返回安全展示字段，`/api/play-url` 只接受清单中的曲目 id。

B2 桶保持私有，Function 使用服务端密钥为对应对象生成短期 GET 签名。签名地址在有效期内是持有者凭据；接口公开不等于对象永久公开。

如果曾在 Cloudflare 平台为播放器域名配置 Cloudflare Access 策略，代码不能移除平台策略；需要在平台取消对播放器域名的保护，避免平台拦截无登录访问。只处理播放器域名，不修改其他应用的策略。

首版不为预览环境配置生产 B2 密钥；预览使用示例曲库或独立测试桶。

### 6.3 必要的凭据边界

- `B2_ENDPOINT`、`B2_REGION` 是普通变量；`B2_BUCKET`、`B2_KEY_ID`、`B2_APPLICATION_KEY` 全部放 Pages Secrets；不得使用 `VITE_` 前缀。
- 签名地址是有效期内的持有者凭据，知道地址的人可以读取对应文件；短 TTL 只能缩短暴露时间，不能即时撤销已发出的地址。
- API 不记录完整签名 URL、Cookie 或 B2 application key；错误日志只写错误代码和曲目 id。
...
下面是后续实现的核心参考，不是已完成的生产接口；调用前必须完成清单查找和配置校验：
...
export async function signTrack(env: B2Env, objectKey: string) {
  ...
  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(objectKey) || objectKey.includes('\\') ||
      parts.some(p => p === '.' || p === '..' || p === '')) {
...
运行依赖限于 React、React DOM、aws4fetch；开发工具采用 Vite、TypeScript、Wrangler，以及框架必需插件/类型。只有准备编写所列必要测试时才引入 Vitest。
...
7. 不自行实现 AWS 签名密码学，不抽象存储适配器、事件总线或通用音频引擎。
...
本地 `.dev.vars` 只供本地 Pages Functions 开发，必须加入 `.gitignore`，不部署、不提交；`.dev.vars.example` 只保留占位符。Production 与 Preview 分别配置两项普通变量和三项 Pages Secrets。使用 Wrangler 时，生产可显式执行 `npx wrangler pages secret put <KEY> --project-name cloud-music --env production`，Preview 执行同一命令并改为 `--env preview`；省略 `--env production` 时默认是 production。不要使用普通 Worker 的 `wrangler secret put`。Pages secrets 可通过 `context.env` 访问。[Pages bindings 与 secrets](https://developers.cloudflare.com/pages/functions/bindings/)、[Pages Wrangler 命令](https://developers.cloudflare.com/workers/wrangler/commands/pages/)、[Pages Wrangler 配置](https://developers.cloudflare.com/pages/functions/wrangler-configuration/)

首次构建 UI 可用显式开发 mock 曲目；生产和预览接口都按公开入口工作，但只为清单中的对象签发短期地址。
...
使只有 API 请求进入 Functions；Functions 目录必须位于项目根目录。`_routes.json` 只负责路由，不负责身份鉴权。[Functions 路由](https://developers.cloudflare.com/pages/functions/routing/)
...
4. 创建 Cloudflare Pages 项目，设置两项普通变量与三项 secrets，配置正式域名。
5. 更新 B2 CORS 正式来源。生产入口开放前，确认访客可以读取曲库并获得合法曲目的短期签名。
...
- 一次完整前后端类型检查和生产构建。
- 一次清单结构校验，不联网逐首探测。
- 少量纯函数/接口用例：中文与空格、`# ? %` key 的签名路径；非法 key/未知 id 拒绝；访客可以得到曲库和合法曲目的短期签名；顺序末尾与单曲随机队列边界。

不设置覆盖率 KPI；不为纯样式和每个按钮编写镜像测试；不建设大规模 E2E、压力测试或整库播放脚本。签名库本身和浏览器音频解码不重复测试。

### 11.2 一次人工冒烟验收

| 检查 | 通过标准 |
| --- | --- |
| 公开入口 | 无登录访客能加载曲库并获得合法曲目的短期播放地址 |
...
当前阶段只交付设计，因此没有声称上述检查已通过。后续完成标准是：真实 B2 文件可以收听和拖动、密钥不进入静态产物、访客只能为清单中的曲目签发短期地址、移动端基本操作通过，即可交付首版。

## 12. 费用、运维与后续触发条件

费用由 B2 存储/实际出站和 Cloudflare Functions 用量组成，以上线账号套餐为准。B2 当前定价页列出按月平均存储量计算的出站免费额度及合作伙伴条件；浏览器直连不能自动套用经 Cloudflare 传输的优惠。[B2 定价](https://www.backblaze.com/cloud-storage/pricing)
...
1. **打通一首真实歌曲**：B2 私有桶、公开接口、签名、audio 播放、Range 拖动。先证明整条链路成立。
2. **完成收听界面**：曲库搜索、底栏、手机布局、队列、收藏。
3. **补齐必要稳定性**：异步竞争、播放拒绝、过期恢复、本机持久化。
4. **构建与发布**：配置 Pages/CORS，执行限定的检查并交付。
...
同步覆盖桶内根目录和任意子目录，忽略非音频对象。每首最多读取前2 MiB，最多3个并发请求；使用 music-metadata 解析标题、歌手、专辑，不下载完整大文件或提取封面。保留人工信息，无标签时使用目录：首层歌手、父级专辑，CD1/CD2归入上一层专辑。根目录无标签时保留未知信息。M4A尾部标签可能超出读取范围。

前端提供专辑筛选，移动端显示歌手和专辑。兼容 B2_ACCESS_KEY_ID/B2_SECRET_ACCESS_KEY 别名，两项凭据和 B2_BUCKET 只放本地配置或 Pages Secret。实际文件路径只保存在服务端清单。

实际联调：579个对象中识别425首音频，整理为29个专辑分组；修复旧GBK标签乱码、双碟名称后缀和目录内少数错误专辑标签。目录内超过半数歌曲使用同一专辑标签时统一该组，其余混合集合保留各自标签，人工编辑的已有专辑优先。

## 密码入口与Safari更新

当前实现以README和docs/DEPLOY.md中的密码部署章节为准：SITE_PASSWORD服务端Secret、签名HttpOnly会话保护全部曲库和播放接口；首页取消搜索栏。原先匿名访问的设计已替换。Safari播放改用同源受保护的302媒体入口，在触摸事件内调用play；音频仍由B2传输。
