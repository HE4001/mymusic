...
播放器使用共享密码登录；通过服务端校验后才能读取曲库和请求播放地址。B2 桶保持私有，B2 密钥只在服务端配置，接口只为曲库清单中的对象签名。

本仓库包含播放器代码、配置示例和维护说明。当前提交的曲库有425首歌曲；实际播放需要有效的 B2 配置和网站密码。历史联调与验收记录统一保存在 [docs/REVIEW.md](docs/REVIEW.md)。
...
B2 使用以下五项配置，网站登录另需 `SITE_PASSWORD` Secret（见下文“网站密码与 iOS Safari”）：
...
如果曾在 Cloudflare 平台为播放器域名配置 Cloudflare Access 策略，代码不能移除平台策略；需要在 Cloudflare 平台取消对播放器域名的保护，避免播放器被平台拦截。只处理播放器域名，不修改其他应用的策略。

## 文档

日常维护以本 README 和部署文档为准；设计指南、任务书及早期审查记录保留作历史参考。
